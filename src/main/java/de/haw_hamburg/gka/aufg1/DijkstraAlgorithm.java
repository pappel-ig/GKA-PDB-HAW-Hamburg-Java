package de.haw_hamburg.gka.aufg1;
...
public class DijkstraAlgorithm {
    ...
    private void initialize(Node v_1) {
        ...
        nodePriorityQueue = new PriorityQueue<>(Comparator.comparingInt(GraphHelper::abstand));
    ...
    private void calculate(final Node v_1) {
        ...
        while (!nodePriorityQueue.isEmpty()) {
            ...
            u.neighborNodes().filter(u::hasEdgeToward).filter(node -> !node.equals(v_1)).forEach(v -> {
                ...
                if (distanceBetweenNode < GraphHelper.abstand(v)) {
    ...
    private int getDistanceBetweenNode(Node u, Node v) {
        return GraphHelper.abstand(u) + GraphHelper.weight(getMinEdgeBetween(u, v));
    }
    ...
    private Edge getMinEdgeBetween(Node u, Node v) {
        return u.leavingEdges()
                ...
                .min(Comparator.comparingInt(GraphHelper::weight))
                .orElseThrow();
    }
}
